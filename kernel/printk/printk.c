...
static bool time_format=0;
module_param_named(uart_format, time_format, bool, S_IRUGO | S_IWUSR);
...
static size_t print_time(u64 ts, char *buf)
{
	...
	if (!buf){
                if (time_format) return snprintf(NULL, 0, "[%5lu.000000] ", (unsigned long)ts);
		return snprintf(NULL, 0, "(%lu)0000-00-00 00:00:00 :", (unsigned long)ts);
        }
	...
	timespec = current_kernel_time();
        ...
        if(time_format) return sprintf(buf, "[%5lu.%06lu] ",(unsigned long)ts, rem_nsec / 1000);

        return sprintf(buf, "(%lu)%04d-%02d-%02d %02d:%02d:%02d :",
                (unsigned long)ts,
