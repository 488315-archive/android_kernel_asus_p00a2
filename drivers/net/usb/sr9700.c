...
static int sr9700_rx_fixup(struct usbnet *dev, struct sk_buff *skb)
{
	...
	while (skb->len > SR_RX_OVERHEAD) {
		...
		/* extract this packet into new skb, and submit */
		sr_skb = netdev_alloc_skb_ip_align(dev->net,len);
		if (!sr_skb) {
			netdev_err(dev->net, "rx buffer allocation failed\n");
			return 0;
		}
		skb_put(sr_skb, len);
...
tiny_rx_frame:
	...
	return 0;
large_rx_frame:
	netdev_err(dev->net, "unexpected large rx frame\n");
	return 0;
}
