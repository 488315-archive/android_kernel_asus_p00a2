...
#include "gt6108_firmware_1002.h"
#endif
...
extern int GT6108_FWForceUpdate;
...
static u8 gup_get_ic_fw_msg(struct i2c_client *client)
{
    ...
    GTP_INFO("IC Product id:%s", update_msg.ic_fw_msg.pid);
    ...
    GTP_INFO("IC version id:%04x", update_msg.ic_fw_msg.vid);
...
static u8 gup_enter_update_judge(st_fw_head *fw_head)
{
            ...
            (!memcmp(fw_head->pid, "91XX", 4)))
    {
        ...
        //The third condition
      	 GTP_INFO("GT6108_FWForceUpdate = %d", GT6108_FWForceUpdate); // add by leo
        if ((fw_head->vid > update_msg.ic_fw_msg.vid)||(GT6108_FWForceUpdate))
        {
            GT6108_FWForceUpdate = 0; // add by leo
...
static s8 gup_update_config(struct i2c_client *client)
{
    ...
    GTP_INFO("[%s]: //////////////////// config version before write.", __func__);
    ret = gt6108_read_cfg_version(i2c_connect_client);
    if(ret < 0)
    {
    ...
    // add by leo for config update check ++
    GTP_INFO("[%s]: //////////////////// config version after write.", __func__);
    msleep(300);
    ret = gt6108_read_cfg_version(i2c_connect_client);
    if(ret < 0)
    {
...
static void gup_search_file(s32 search_type)
{
    ...
    for (i = 0; i < GUP_SEARCH_FILE_TIMES; ++i)
    {
        ...
        {
            GTP_INFO("Search for %s, %s for fw update.(%d/%d)", UPDATE_FILE_PATH_1, UPDATE_FILE_PATH_2, i+1, GUP_SEARCH_FILE_TIMES);
        ...
        {
            GTP_INFO("Search for %s, %s for config update.(%d/%d)", CONFIG_FILE_PATH_1, CONFIG_FILE_PATH_2, i+1, GUP_SEARCH_FILE_TIMES);
...
static u8 gup_check_update_file(struct i2c_client *client, st_fw_head* fw_head, u8* path)
{
    ...
    GTP_INFO("[%s]: start", __func__); // add by leo 

    got_file_flag = 0x00;
    if (path)
    {
        //GTP_DEBUG("[%s]: Update File path:%s, %d", __func__ , path, strlen(path)); // mark by leo for 8916
    ...
    else
    {
        ...
        if (got_file_flag & CFG_FILE_READY)
        {
            ret = gup_update_config(i2c_connect_client); // update config from data file
            if(ret <= 0)
            {
...
s32 gup_update_proc(void *dir)
{
    ...
    GTP_INFO("[%s]: //////////////////// gup_check_update_file.", __func__);

    update_msg.file = NULL;
    ret = gup_check_update_file(i2c_connect_client, &fw_head, (u8*)dir);	// update config
    if(FAIL == ret)
    {
    ...
    GTP_INFO("[%s]: //////////////////// gup_get_ic_fw_msg.", __func__);
    ...
    GTP_INFO("[%s]: //////////////////// gup_enter_update_judge.", __func__);
    ...
    GTP_INFO("[%s]: //////////////////// gup_enter_update_mode.", __func__);
    ...
    GTP_INFO("[%s]: //////////////////// start to update...", __func__);
    while(retry++ < 5)
    {
        show_len = 10;
        total_len = 100;
    ...
    // add by leo ++
    GTP_INFO("[%s]: //////////////////// gup_check_update_file 2.", __func__);
