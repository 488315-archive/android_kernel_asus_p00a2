...
static int mmc_select_timing(struct mmc_card *card)
{
	...
	if (card->mmc_avail_type & EXT_CSD_CARD_TYPE_HS200){
		clktdelay_mode = "mmc-hs200-1_8v";
		card->host->ops->clk_delay_tuning(card->host, clktdelay_mode);
	}
...
skip_cache:
	...
	if( (card->ext_csd.rev) == 8 //5.1
	     && (card->cid.manfid) == 21 //samsung
		 && ((card->raw_cid[2] >> 16) & 0xff) == 7) //fw=7
	{
		pr_info("Enable cache flush before switch partition.\n");
		card->cache_flush_before_switch = 1;
	}
	else
		card->cache_flush_before_switch = 0;
