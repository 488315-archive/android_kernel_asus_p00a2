				      ...
				      struct mmc_blk_data *md)
{
	...
	if (mmc_card_mmc(card)) {
		...
		if((card->cache_flush_before_switch == 1) && (part_config == EXT_CSD_PART_CONFIG_ACC_RPMB)){
			pr_info("%s cache flush before switch. \n", __func__);
			err = mmc_flush_cache(card);
			if (err)
				pr_err("%s cache flush failed. (%x)\n", __func__, err);
		}
