...
//ASUS_BSP+++, Fix green screen
/*
Patch root cause:
It fixes the coding defects of increasing ISP reference count "twice" at first time open ISP device during boot time.
Since its ref. count is added one more time, that's why isp driver can't be released & init. correctly.
And the camera memory info. is also not initial/clear in open camera flow.
Finally it causes EMI violation at camera dma ports and also makes system HWT.
*/
#if 0
	g_IspInfo.UserCount++;
