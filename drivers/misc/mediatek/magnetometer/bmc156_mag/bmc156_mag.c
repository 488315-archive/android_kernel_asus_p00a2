...
static int g_config_atd = 0;	// for ATD test init
...
static int bmm150_checkchipid(struct i2c_client *client)
{
	...
	if(chip_id!=SENSOR_CHIP_ID_BMM){
		...
		MSE_LOG("retry to read chip id result: %#x", chip_id);
	}
...
static ssize_t bmm_show_value(struct device *dev, struct device_attribute *attr, char *buf)
{
	int sensordata[3], iloop=0;
	char strbuf[BMM150_BUFSIZE];
	if(g_config_atd==0){
...
{
	MSE_LOG("show bmm status !!\n");
	return sprintf(buf, "%d\n", bmm150_init_flag);
}
static DEVICE_ATTR(value, 0660, bmm_show_value, NULL);
...
static int bmm150_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (err != 0) {
		MSE_ERR("create mag attribute err = %d\n", err);
		goto exit_kfree;
	}	
	// sys init --
