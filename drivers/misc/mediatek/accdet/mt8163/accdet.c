...
// joe_cheng : gpio 88 for headset and speaker switch
static unsigned int pin_headsetspk;
...
/* joe_cheng : Z300M headset and speaker switch
   (1) Set gpio 88 to low always when headset is connected
   (2) Add this delay function to avoid the burst from speaker to headset
 */
static void set_spk_hs_gpio_work(struct work_struct *work)
{
	if (pin_headsetspk != 0){
		pr_debug("%s : cable type %d\n", __func__, cable_type);
		if (cable_type == 0)
			gpio_set_value(pin_headsetspk, 1);
		else
			gpio_set_value(pin_headsetspk, 0);
	}
}
static DECLARE_DELAYED_WORK(spk_hs_gpio_work, set_spk_hs_gpio_work);
...
static inline void headset_plug_out(void)
{
	...
	// Z300M : headset and speaker switch
	if(PROJ_ID == 0x1){ //Z300M headset spk switch
		schedule_delayed_work(&spk_hs_gpio_work, msecs_to_jiffies(100));
	}
	...
	if (node) {
		...
		if (ret != 0) {
			...
			ACCDET_ERROR("[Accdet]accdet set EINT finished, accdet_irq=%d, headsetdebounce=%d\n",
                ...
                if(PROJ_ID == 0x1){ //Z300M headset spk switch
                        pin_headsetspk = of_get_named_gpio(node, "headsetspk-gpio", 0);
                        if (pin_headsetspk != 0) {
                                pr_info("%s : headset and speaker switch %d\n", __func__, pin_headsetspk);
                        } else {
                                pr_err("%s : get headset and speaker switch gpio pin fail!!!\n", __func__);
                        }
                }     
	} else {
...
static void accdet_work_callback(struct work_struct *work)
{
	...
	if (1 == eint_accdet_sync_flag) {
		...
		// joe_cheng : Z300M headset and speaker switch
		if(PROJ_ID == 0x1){
			schedule_delayed_work(&spk_hs_gpio_work, msecs_to_jiffies(150));
		}
	} else {
...
int mt_accdet_probe(struct platform_device *dev)
{
	...
	if (g_accdet_first == 1) {
        ...
        if(PROJ_ID == 0x1){ //Z300M headset spk switch
                ret = gpio_request(pin_headsetspk, "headset-spk-switch");
                if (ret < 0) {
                        pr_err("%s() request headset spk switch failed\n", __func__);
                } else {
                        printk("%s() request headset spk switch successfully\n", __func__);
                }
                gpio_direction_output(pin_headsetspk, 1);
        }
