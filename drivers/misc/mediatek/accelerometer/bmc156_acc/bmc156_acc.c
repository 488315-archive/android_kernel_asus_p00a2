...
static void bma2x2_read_from_califile(int *calidata); 
...
static ssize_t bma_show_value(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = bma255_i2c_client;
	char strbuf[BMA255_BUFSIZE];

	GSE_LOG("bma_show_value !!\n");
	if(NULL == client)
	{
		GSE_ERR("i2c client is null!!\n");
		return 0;
...
static ssize_t bma_show_state(struct device *dev, struct device_attribute *attr, char *buf)
{
	GSE_LOG("show bma status !!\n");
	return sprintf(buf, "%d\n", bma255_init_flag);
}

static ssize_t bma_do_cali(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = bma255_i2c_client;
	struct bma255_i2c_data *obj = (struct bma255_i2c_data*)i2c_get_clientdata(client);
	int acc[BMA255_AXES_NUM]={0};
	int cali[BMA255_AXES_NUM]={0};
	...
	for(index=0;index<iloop;index++){
		GSE_LOG("bma_do_cali  %s !!\n", strbuf);
		res = BMA255_ReadData(client, databuf);
		acc[obj->cvt.map[BMA255_AXIS_X]] = obj->cvt.sign[BMA255_AXIS_X]*databuf[BMA255_AXIS_X];
		acc[obj->cvt.map[BMA255_AXIS_Y]] = obj->cvt.sign[BMA255_AXIS_Y]*databuf[BMA255_AXIS_Y];
		acc[obj->cvt.map[BMA255_AXIS_Z]] = obj->cvt.sign[BMA255_AXIS_Z]*databuf[BMA255_AXIS_Z];
#ifdef CONFIG_Z380M
		cali[BMA255_AXIS_X]=cali[BMA255_AXIS_X]+acc[obj->cvt.map[BMA255_AXIS_X]] ;
		cali[BMA255_AXIS_Y]=cali[BMA255_AXIS_Y]+acc[obj->cvt.map[BMA255_AXIS_Y]] ;
		cali[BMA255_AXIS_Z]=cali[BMA255_AXIS_Z]+acc[obj->cvt.map[BMA255_AXIS_Z]] ;
#endif
#ifdef CONFIG_Z300M
		cali[BMA255_AXIS_X]=cali[BMA255_AXIS_X]+acc[obj->cvt.map[BMA255_AXIS_Y]] ;
		cali[BMA255_AXIS_Y]=cali[BMA255_AXIS_Y]+acc[obj->cvt.map[BMA255_AXIS_X]] ;
		cali[BMA255_AXIS_Z]=cali[BMA255_AXIS_Z]+acc[obj->cvt.map[BMA255_AXIS_Z]] ;
#endif
	}
	...
	cali[BMA255_AXIS_X]=(0-cali[BMA255_AXIS_X]/iloop);
	cali[BMA255_AXIS_Y]=(0-cali[BMA255_AXIS_Y]/iloop);
	cali[BMA255_AXIS_Z]=(512-cali[BMA255_AXIS_Z]/iloop);
	GSE_LOG("- calibration data x:%d, y:%d, z:%d  !!\n",cali[BMA255_AXIS_X],cali[BMA255_AXIS_Y],cali[BMA255_AXIS_Z]);
	if(pre_enable==0){
#ifdef CONFIG_CUSTOM_KERNEL_SENSORHUB
		BMA255_SCP_SetPowerMode(false, ID_ACCELEROMETER);
#else
		BMA255_SetPowerMode(client, false);
#endif
	}
	if( (cali[BMA255_AXIS_X]>250)||(cali[BMA255_AXIS_X]<-250) ){
		GSE_LOG("sensitivity X out of range !!!\n");
		return sprintf(buf, "%d\n", 0);
	}
	else if( (cali[BMA255_AXIS_Y]>250)||(cali[BMA255_AXIS_Y]<-250) ){
		GSE_LOG("sensitivity Y out of range !!!\n");
		return sprintf(buf, "%d\n", 0);
	}
	else if( (cali[BMA255_AXIS_Z]>250)||(cali[BMA255_AXIS_Z]<-250) ){
		GSE_LOG("sensitivity Z out of range !!!\n");
		return sprintf(buf, "%d\n", 0);
	}else{		
		BMA255_WriteCalibration(bma255_i2c_client,cali);
		bma2x2_write_to_califile(cali[BMA255_AXIS_X],cali[BMA255_AXIS_Y],cali[BMA255_AXIS_Z]);
	}
	return sprintf(buf, "%d\n", 1);
}

static ssize_t bma_rst_cali(struct device *dev, struct device_attribute *attr, char *buf)
{
	GSE_LOG("bma_rst_cali !!\n");
	BMA255_ResetCalibration(bma255_i2c_client);
	return 0;
}

static ssize_t bma_atd_cali(struct device *dev, struct device_attribute *attr, char *buf)
{
	int calivalue[3]={0};
	GSE_LOG("bma_atd_cali !!\n");
	bma2x2_read_from_califile(calivalue);
	return sprintf(buf, "%d %d %d\n", calivalue[0], calivalue[1], calivalue[2]);
}

/*----------------------------------------------------------------------------*/
static ssize_t bma_show_power_state(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", sensor_power);
}

static ssize_t bma_set_power_state(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
#ifdef CONFIG_CUSTOM_KERNEL_SENSORHUB
#else
	struct i2c_client *client = bma255_i2c_client;
	...
	if(buf[0]=='1'){
		...
		BMA255_SetPowerMode(client, false);
#endif
		GSE_LOG("set power off\n");
	}

	return count;
}

static DEVICE_ATTR(value, 0660, bma_show_value, NULL);
static DEVICE_ATTR(state, 0660, bma_show_state, NULL);
static DEVICE_ATTR(gs_cali, 0660, bma_do_cali, NULL);
static DEVICE_ATTR(resetcali, 0660, bma_rst_cali, NULL);
static DEVICE_ATTR(atd_caligs, 0660, bma_atd_cali, NULL);
static DEVICE_ATTR(power, 0660, bma_show_power_state, bma_set_power_state);

//static struct attribute *bmm_attributes[] = {
//	&dev_attr_value.attr,
//	&dev_attr_state.attr,
//	&dev_attr_gs_cali.attr,
//	&dev_attr_resetcali.attr,
//	&dev_attr_atd_caligs.attr,
//	NULL
//};
...
static int bma255_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	err += sysfs_create_file(android_gsensor_kobj, &dev_attr_state.attr);
	err += sysfs_create_file(android_gsensor_kobj, &dev_attr_gs_cali.attr);
	err += sysfs_create_file(android_gsensor_kobj, &dev_attr_resetcali.attr);
	err += sysfs_create_file(android_gsensor_kobj, &dev_attr_atd_caligs.attr);
	err += sysfs_create_file(android_gsensor_kobj, &dev_attr_power.attr);
	if (err) {
