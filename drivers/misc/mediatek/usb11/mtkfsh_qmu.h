...
extern int mtkfsh_qmu_dbg_level;	/* refer to musbfsh_core.c */
static inline int mtkfsh_dbg_level(unsigned level)
{
	return mtkfsh_qmu_dbg_level >= level;
}
...
#define QMU_DBG_ON
#ifdef QMU_DBG_ON
#define QMU_ERR(format, args...) do {if (mtkfsh_dbg_level(LOG_ERR)) \
	pr_warn("QMU_ERR,<%s %d>, " format , __func__, __LINE__ , ## args);  } \
	while (0)
#define QMU_WARN(format, args...) do {if (mtkfsh_dbg_level(LOG_WARN)) \
	pr_warn("QMU_WARN,<%s %d>, " format , __func__, __LINE__ , ## args);  } \
	while (0)
#define QMU_INFO(format, args...) do {if (mtkfsh_dbg_level(LOG_INFO)) \
	pr_warn("QMU_INFO,<%s %d>, " format , __func__, __LINE__ , ## args);  } \
	while (0)
#define QMU_DBG(format, args...) do {if (mtkfsh_dbg_level(LOG_DBG)) \
...
#define TGPD_SET_CHKSUM(_pd, _n)    (((TGPD *)_pd)->chksum = musbfsh_PDU_calcCksum((u8 *)_pd, _n))
#define TGPD_SET_CHKSUM_HWO(_pd, _n)    (((TGPD *)_pd)->chksum = musbfsh_PDU_calcCksum((u8 *)_pd, _n)-1)
...
extern void mtkfsh_qmu_destroy_gpd_pool(struct device *dev);
extern int mtkfsh_qmu_init_gpd_pool(struct device *dev);
extern void mtkfsh_qmu_reset_gpd_pool(u32 ep_num, u8 isRx);
extern bool mtkfsh_is_qmu_enabled(u8 ep_num, u8 isRx);
//extern void mtkfsh_qmu_enable(struct musbfsh* musbfsh, u8 ep_num, u8 isRx);
extern void mtkfsh_qmu_insert_task(u8 ep_num, u8 isRx, u8* buf, u32 length, u8 zlp);
extern void mtkfsh_qmu_resume(u8 ep_num, u8 isRx);
#if 0 //gadget
extern void qmu_done_rx(struct musbfsh *musbfsh, u8 ep_num);
extern void qmu_done_tx(struct musbfsh *musbfsh, u8 ep_num);
#endif
//extern void mtkfsh_qmu_irq_err(struct musbfsh *musbfsh, u32 qisar);
//extern void mtkfsh_flush_ep_csr(struct musbfsh* musbfsh, u8 ep_num, u8 isRx);
extern void mtkfsh_qmu_stop(u8 ep_num, u8 isRx);
//extern void h_qmu_done_rx(struct musbfsh *musbfsh, u8 ep_num);
//extern void h_qmu_done_tx(struct musbfsh *musbfsh, u8 ep_num);
extern void mtkfsh_qmu_insert_task_ioc(u8 ep_num, u8 isRx, u8* buf, u32 length, u8 zlp,u8 isioc);
#endif
