 ...
 * ---------------------------------------------------------------------------
 */
static ssize_t show_lcmid(struct device *dev, struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", primary_display_get_lcm_index());
}

static DEVICE_ATTR(lcmid, S_IRUGO, show_lcmid, NULL);
...
static int mtkfb_register_sysfs(struct mtkfb_device *fbdev)
{
	...
	ret = device_create_file(fbdev->dev, &dev_attr_lcmid);
	if (ret) {
		pr_err("failed to create sysfs lcmid file\n");
		return ret;
	}
...
{
	/*NOT_REFERENCED(fbdev);*/
	device_remove_file(fbdev->dev, &dev_attr_lcmid);
}
...
#ifdef CONFIG_OF
	//mike: pass lk lcm_name to kernel
