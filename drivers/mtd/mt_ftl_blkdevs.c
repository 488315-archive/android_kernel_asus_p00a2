...
#if 0 /* TO DO*/
int mt_ftl_thread(void *u)
{
	int failures = 0;
	struct mt_ftl_blk *ftl = u;

	mt_ftl_err("background thread started, PID %d", task_pid_nr(current));
	...
	for (;;) {
		...
		failures = 1;
		err = 0;
		ftl->thread_enabled = 1;
		cond_resched();
	}

	mt_ftl_err("background thread is killed");
	return 0;
}
#endif
				     ...
				     const struct kernel_param *kp)
{
	...
	if (len == 0) {
		mt_ftl_debug("block: empty 'block=' parameter - ignored\n");
		return 0;
	}

	if (len == MT_FTL_BLK_PARAM_LEN) {
		mt_ftl_err("block: parameter \"%s\" is too long, max. is %d\n", val, MT_FTL_BLK_PARAM_LEN);
		return -EINVAL;
	}
...
static int mt_ftl_blk_flush(struct mt_ftl_blk *dev, bool sync)
{
	...
	if (ret) {
		mt_ftl_err(dev, "mt_ftl_commit error %d\n", ret);
		return ret;
	} */
...
static int do_mt_ftl_blk_request(struct mt_ftl_blk *dev, struct request *req)
{
	...
	if ((req->bio->bi_rw & WRITE_FLUSH) == WRITE_FLUSH)
		dev->flush = 1;
	else if ((req->bio->bi_rw & WRITE_SYNC) == WRITE_SYNC)
		dev->sync = 1;

	if (unlikely(req->bio->bi_rw & REQ_DISCARD)) {
		mt_ftl_err("REQ_DISCARD to mt_ftl");
...
static int mt_ftl_blk_open(struct block_device *bdev, fmode_t mode)
{
	...
	if (IS_ERR(dev->desc)) {
		mt_ftl_err("%s failed to open ubi volume %d_%d",
...
int mt_ftl_blk_create(struct ubi_volume_desc *desc)
{
	...
	dev->pm_per_io = dev->min_io_size >> PM_PER_NANDPAGE_SHIFT;

	pr_crit("Device info:\n\tubi_num:%d vol_id:%d name:%s leb_size:%d\n\tmin_io_size:%d pm_per_io:%d dev_blocks:%d\n",
		dev->ubi_num, dev->vol_id, vi.name, dev->leb_size, dev->min_io_size, dev->pm_per_io, dev->dev_blocks);

	/* Initialize the gendisk of this mt_ftl_blk device */
	gd = alloc_disk(1);
	if (!gd) {
		mt_ftl_err("block: alloc_disk failed");
		ret = -ENODEV;
		goto out_free_dev;
	}
	...
	if (!dev->rq) {
		mt_ftl_err("block: blk_init_queue failed");
		ret = -ENODEV;
		goto out_put_disk;
	}
...
static int mt_ftl_blk_resize(struct ubi_volume_info *vi)
{
	...
	if ((sector_t)disk_capacity != disk_capacity) {
		mutex_unlock(&devices_mutex);
		mt_ftl_debug("%s: the volume is too big (%d LEBs), cannot resize", dev->gd->disk_name, vi->size);
		return -EFBIG;
	}
...
static int __init mt_ftl_blk_create_from_param(void)
{
	...
	for (i = 0; i < mt_ftl_blk_devs; i++) {
		...
		if (IS_ERR(desc)) {
			mt_ftl_err("block: can't open volume, err=%ld\n", PTR_ERR(desc));
			ret = PTR_ERR(desc);
			break;
		}
		...
		if (ret) {
			mt_ftl_err("block: can't add '%s' volume, err=%d\n", vi.name, ret);
			break;
		}
