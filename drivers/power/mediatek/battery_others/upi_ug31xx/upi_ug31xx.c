...
extern int g_cover_gauge_init;
extern int g_cover_low_bat_gauge_check;
#else
...
static void upi_ug31xx_main_task(struct work_struct *work)
{
  ...
  if(wSysMode & SYS_MODE_INIT_CAP)
  {
	...
	g_cover_gauge_init = 1;
	GLOGE("[%s]: g_cover_gauge_init = %d\n", __func__, g_cover_gauge_init);
  ...
  {
	if(g_cover_low_bat_gauge_check < 10){
		GLOGE("[%s] Check Low Battery initial fail(%d)\n", __func__,g_cover_low_bat_gauge_check);
		g_cover_low_bat_gauge_check++;
	}else{
		GLOGE("[%s] Check Low Battery initial fail(%d)\n", __func__,g_cover_low_bat_gauge_check);
		upi_ug31xx->charging_stopped = false;
		start_charging();
