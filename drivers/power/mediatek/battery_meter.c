...
#else
#include <mach/mul_battery_meter_table.h>
#endif
...
//static bool Debug_SOC_Flag = true;
...
void fgauge_get_profile_id(void)
{
	...
	if (ret != 0){
		battery_log(BAT_LOG_CRTI, "[fgauge_get_profile_id]id_volt read fail\n");
		bm_print(BM_LOG_CRTI, "[fgauge_get_profile_id]id_volt read fail\n");
	}
	else{
		battery_log(BAT_LOG_CRTI, "[fgauge_get_profile_id]id_volt = %d\n", id_volt);
	...
	if ((sizeof(g_battery_id_voltage) / sizeof(signed int)) != TOTAL_BATTERY_NUMBER) {
		battery_log(BAT_LOG_CRTI, "[fgauge_get_profile_id]error! voltage range incorrect!\n");
	...
	battery_log(BAT_LOG_CRTI, "[fgauge_get_profile_id]Battery id (%d)\n", g_fg_battery_id);
...
#ifdef MTK_MULTI_BAT_PROFILE_SUPPORT
		Batt_Temperature_Table[idx / 2][0].BatteryTemp = addr;
		Batt_Temperature_Table[idx / 2][0].TemperatureR = val;
#else
...
if (((g_rtc_fg_soc != 0)
	...
	battery_log(BAT_LOG_CRTI, "[%s][FGADC] Before change, g_rtc_fg_soc=%d, gFG_capacity_by_v=%d\n",
                 __func__,g_rtc_fg_soc, gFG_capacity_by_v);//Original MTK get ui_soc for dod0.
...
/*	      || (abs(g_rtc_fg_soc - g_booting_vbat) < batt_meter_cust_data.cust_poweron_delta_capacity_tolrance)*/)
	     &&
	     ((gFG_capacity_by_v > batt_meter_cust_data.cust_poweron_low_capacity_tolrance || bat_is_charger_exist() == KAL_TRUE)))
...
/*		|| ((g_rtc_fg_soc != 0)
	...
	sw_oam_init_v2();
#endif
	battery_log(BAT_LOG_CRTI, "[%s] After change, g_rtc_fg_soc=%d, gFG_capacity_by_v=%d\n",
		__func__,g_rtc_fg_soc, gFG_capacity_by_v);//Original MTK get ui_soc for dod0
...
void oam_init(void)
{
	...
	battery_log(BAT_LOG_CRTI,"[%s] OCV(%d), capacity_OCV(%d), g_booting_vbat(%d), vbat_capacity(%d)\n",
					__func__, gFG_voltage, gFG_capacity_by_v, g_booting_vbat,vbat_capacity);
...
void oam_run(void)
{
	...
	/*AC charging will suspend, USB charging won't*/
	/*oam_run is used in active*/
	battery_log(BAT_LOG_CRTI,"chr_wake_up_for_meter_check = %d\n",chr_wake_up_for_meter_check);
	if(bat_is_charger_exist() == KAL_TRUE && (chr_wake_up_for_meter_charging == KAL_FALSE) && (chr_wake_up_for_meter_discharging == KAL_TRUE)){
		if((chr_wake_up_for_meter_check == KAL_TRUE)){//plug in cable
			getrawmonotonic(&now_time);
			battery_log(BAT_LOG_CRTI,"[%s]Plug In cable, Wake time\n",__func__);
		}else{//charging
			get_monotonic_boottime(&now_time);
			battery_log(BAT_LOG_CRTI,"[%s]Charging, Sleep+Wake time\n",__func__);
		}
	}else if(bat_is_charger_exist() == KAL_FALSE && (chr_wake_up_for_meter_charging == KAL_TRUE) && (chr_wake_up_for_meter_discharging == KAL_FALSE)){
		if(chr_wake_up_for_meter_check == KAL_TRUE){//plug out cable
			get_monotonic_boottime(&now_time);
			battery_log(BAT_LOG_CRTI,"[%s]Plug Out cable, Sleep+Wake time\n",__func__);
		}else{//discharging
			getrawmonotonic(&now_time);
			battery_log(BAT_LOG_CRTI,"[%s]Discharging, Wake time\n",__func__);
		}
	}else{
		battery_log(BAT_LOG_CRTI,"[%s]Special Case, Ignore!!\n",__func__);
		now_time.tv_sec = last_oam_run_time.tv_sec;
	}
	/* delta_time = now_time - last_oam_run_time; */
	delta_time = now_time.tv_sec - last_oam_run_time.tv_sec;
	battery_log(BAT_LOG_CRTI,"[oam_run_time] now_time = %ld, last_time = %ld.\n",now_time.tv_sec,last_oam_run_time.tv_sec);

#if defined(SW_OAM_INIT_V2)
	printk(bootbuf);
#endif
	if(chr_wake_up_for_meter_check == KAL_TRUE){
		if(bat_is_charger_exist() == KAL_TRUE &&
			chr_wake_up_for_meter_charging == KAL_FALSE &&
			chr_wake_up_for_meter_discharging == KAL_TRUE){//plug in cable
			get_monotonic_boottime(&last_oam_run_time);
			battery_log(BAT_LOG_CRTI,"[%s]Plug In cable, change last_oam_run_time for next oam_run\n",__func__);
		}else if(bat_is_charger_exist() == KAL_FALSE &&
			chr_wake_up_for_meter_charging == KAL_TRUE &&
			chr_wake_up_for_meter_discharging == KAL_FALSE){//plug out cable
			getrawmonotonic(&last_oam_run_time);
			battery_log(BAT_LOG_CRTI,"[%s]Plug Out cable, change last_oam_run_time for next oam_run\n",__func__);
		}
	}else{
		last_oam_run_time = now_time;
		battery_log(BAT_LOG_CRTI,"[%s]Charging/Discharging State\n",__func__);
	}
	battery_log(BAT_LOG_CRTI,"[oam_run_time] last_time = %ld.\n",last_oam_run_time.tv_sec);
...
//	if(Debug_SOC_Flag == true){
		chr_wake_up_for_meter_check = 0;
		battery_log(BAT_LOG_CRTI,"[%s]gFG_DOD0 = %d,oam_v_ocv_2 = %d,vol_bat = %d,oam_i_2 = %d,oam_r_2 =%d,oam_car_2 = %d,Cmax = %d,oam_d_2 = %d, chr_wake_up_for_meter_check = %d\n",
			__func__,gFG_DOD0,oam_v_ocv_2,vol_bat,oam_i_2,oam_r_2,oam_car_2,gFG_BATT_CAPACITY_aging,oam_d_2,chr_wake_up_for_meter_check);
//	}
	bm_print(BM_LOG_CRTI, "[oam_init_inf] %d, %d, %d, %d, %d, %d\n",
		 gFG_voltage, (100 - fgauge_read_capacity_by_v(gFG_voltage)), g_rtc_fg_soc,
		 gFG_DOD0, oam_v_ocv_init, force_get_tbat(KAL_FALSE));
...
/* ============================================================ // */
#if 0
static ssize_t show_Debug_SOC(struct device *dev, struct device_attribute *attr, char *buf)
{
        battery_log(BAT_LOG_CRTI, "[%s]  Debug_SOC_Flag = %d\n", __func__,Debug_SOC_Flag);
        return sprintf(buf, "%d\n", Debug_SOC_Flag);
}

static ssize_t store_Debug_SOC(struct device *dev, struct device_attribute *attr,
                                      const char *buf, size_t size)
{
        int request;
        sscanf(buf, "%du", &request);
        if (!request){
                Debug_SOC_Flag = false;
                battery_log(BAT_LOG_CRTI, "[%s]  Debug_SOC_Flag = %d\n", __func__,Debug_SOC_Flag);
        }else{
                Debug_SOC_Flag = true;
                battery_log(BAT_LOG_CRTI, "[%s]  Debug_SOC_Flag = %d\n", __func__,Debug_SOC_Flag);
...
static DEVICE_ATTR(Debug_SOC, 0664, show_Debug_SOC, store_Debug_SOC);
#endif
...
/* ============================================================ // */
//++ shilun_huang@asus.com add for atd test
static ssize_t show_gaugeIC_status(struct device *dev, struct device_attribute *attr, char *buf)
{
    signed int vbat = 0;
...
static ssize_t store_gaugeIC_status(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
    return size;
}
static DEVICE_ATTR(gaugeIC_status, 0644, show_gaugeIC_status, store_gaugeIC_status);
/* -------------------------------------------------------------------------------------------- */
static ssize_t show_chargerIC_status(struct device *dev, struct device_attribute *attr, char *buf)
{
    size_t ret =0;
    
    return sprintf(buf, "%zd\n", ret);
}
static ssize_t store_chargerIC_status(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
    return size;
}
static DEVICE_ATTR(chargerIC_status, 0644, show_chargerIC_status, store_chargerIC_status);
//--
...
//	ret_device_file = device_create_file(&(dev->dev), &dev_attr_Debug_SOC);
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_gaugeIC_status);
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_chargerIC_status);
	...
	if (sleep_interval > 1800 && bat_is_charger_exist() == KAL_FALSE) {
		...
		if (hw_ocv_after_sleep < g_hw_ocv_before_sleep) {
			...
			bm_print(BM_LOG_CRTI,"[self-discharge check] reset to HWOCV. dod_ocv(%d) dod_now(%d)\n",
				DOD_hwocv, oam_d_2);
		} else {/*assume 17mA in suspend*/
			oam_car_1 = oam_car_1 +
				(170 * (sleep_interval) / 3600);
/* 0.1mAh */
			oam_car_2 = oam_car_2 +
				(170 * (sleep_interval) / 3600);
/* 0.1mAh */
		...
		oam_car_1 = oam_car_1 + (170*sleep_interval/3600);
		oam_car_2 = oam_car_2 + (170*sleep_interval/3600);
	}
	/* FIXME */
