...
#else
	if (BMT_status.SOC > 65 && b_eng_charging_limit == true){//the condition of stop charging
#endif
...
void mt_battery_GetBatteryData(void)
{
	...
	battery_log(BAT_LOG_CRTI,
		    "AvgVbat=(%d),Avg10_Vbat=(%d),bat_vol=(%d),AvgI=(%d),I=(%d),VChr=(%d),AvgT=(%d),T=(%d),pre_SOC=(%d),SOC=(%d),UI-SOC=(%d),ZCV=(%d),cover_SOC=(%d),cover_vol=(%d)\n",
		    BMT_status.bat_vol, Avg10_bat_vol, bat_vol, BMT_status.ICharging, ICharging,
...
#else
	battery_log(BAT_LOG_CRTI,
		    "AvgVbat=(%d),Avg10_Vbat=(%d),bat_vol=(%d),AvgI=(%d),I=(%d),VChr=(%d),AvgT=(%d),T=(%d),pre_SOC=(%d),SOC=(%d),UI-SOC=(%d),ZCV=(%d)\n",
		    BMT_status.bat_vol, Avg10_bat_vol, bat_vol, BMT_status.ICharging, ICharging,
		    BMT_status.charger_vol, BMT_status.temperature, temperature,
...
//                battery_log(BAT_LOG_CRTI," [%s] Charging = TRUE\n",__func__);
                if ((BMT_status.charger_type == NONSTANDARD_CHARGER) ||(BMT_status.charger_type == STANDARD_CHARGER)||COVER_ATTACHED()){//AC?
#if defined(CONFIG_Z380M)
			if(COVER_ATTACHED())
